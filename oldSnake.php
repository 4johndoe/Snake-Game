...
<html lang="en">
	...
	<body>
		...
		<input type="text" id="scoreLabel" value="0">
		<div id="Board">
...
<script>
	...
	function callFood(k) {
		foodX = randomNumbergen(1, BoadrSize1x - 10);
		foodX = foodX - (foodX%10);

		foodY = randomNumbergen(1, BoadrSize1y - 10);
	...
	function redrawFood() {
		...
		{
			var answer = confirm("Voilaaaa! You won the game!!!");
			alert("Bye bye!");
			window.location = "index.html";
		}
		else if (Score == level2Score) 
		{
			var answer = confirm("Yippie! You cleared 2nd level ... do you want to continue?");
			if (answer) {
	...
	// generate a random number between range x, y
	function randomNumbergen(x, y) {	//foodX = randomNumbergen(1, BoadrSize1x - 10);
	...
	Snake.prototype.draw = function(){
		 ...
		 if (!moveOne) 
		 {
		 	...
		 	for (var i = snakePart; !snakeDead && i > 0; --i) 
		 	{
		 		if (
		 				((document.getElementById("snake" + i).style.left == ((this.x) + "px")) && 
		 				(document.getElementById("snake" + i).style.top == ((this.y) + "px")))
		 			) {
		 			snakeDead = true;
		 		};
		 	};
		 	...
		 	for (var i = 1; i <= Goals; i++) {
	 			...
	 			{
	 				// dynamicaly generate snake size
	 				++snakePart;
	 				...
	 				// to disapear food that is eaten up
